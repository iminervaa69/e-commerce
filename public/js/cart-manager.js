...
// ===== CHECKOUT FUNCTIONS (Your preferred implementation) =====
...
function handleCheckoutSelected() {
    ...
    proceedToCheckout(selectedItems, 'selected');
}

function handleCheckoutAll() {
    const allItems = getAllItemIds();
    proceedToCheckout(allItems, 'all');
}

function proceedToCheckout(itemIds, type) {
    fetch('/cart/proceed-to-checkout', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
...
function initializeCartSystem() {
    ...
    // Checkout buttons - Using your preferred implementation
    const checkoutSelectedBtn = document.getElementById('checkout-selected-btn');
    const checkoutAllBtn = document.getElementById('checkout-all-btn');

    if (checkoutSelectedBtn) {
        checkoutSelectedBtn.addEventListener('click', handleCheckoutSelected);
    }
