{{-- resources/views/frontend/checkout/components/address-selector.blade.php --}}
<div x-data="{ selectedAddress: null, addresses: [] }" x-init="loadAddresses()" class="address-selector">
    <!-- Saved Addresses -->
    <div class="space-y-4">
        <!-- Loading state -->
        <div x-show="addresses.length === 0" class="flex items-center justify-center py-8">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
            <span class="ml-2 text-gray-600 dark:text-gray-400">Loading addresses...</span>
        </div>
        ...
        <template x-for="address in addresses" :key="address.id">
            <label class="block">
                <div class="flex items-start p-4 border-2 rounded-lg cursor-pointer transition-colors duration-300"
                    ...
                    <input type="radio" name="shipping_address" :value="address.id" x-model="selectedAddress" class="mt-1 text-blue-600">
        ...
        <div x-show="addresses.length === 0" class="text-center py-8">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No addresses found</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by adding your first shipping address.</p>
            <div class="mt-6">
